...
def do_upgrade():
    ...
    try:
        ...
        if cfg['CFG_BIBINDEX_REMOVE_HTML_MARKUP']:
            if cfg['CFG_BIBINDEX_REMOVE_HTML_MARKUP'] == 1:
                run_sql("UPDATE idxINDEX SET remove_html_markup='Yes'")
        if cfg['CFG_BIBINDEX_REMOVE_LATEX_MARKUP']:
            if cfg['CFG_BIBINDEX_REMOVE_LATEX_MARKUP'] == 1:
