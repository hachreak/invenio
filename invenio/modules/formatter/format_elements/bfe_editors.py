...
def format_element(bfo, limit, separator=' ; ', extension='[...]',
                   print_links="yes"):
    """Print the list of editors of a record.
    ...
    :param print_links: if yes, print the editors as HTML link to their
           publications
    """
    ...
    editors = [
        bibrecord.field_get_subfield_values(author, 'a')[0]
        for author in authors
        if len(bibrecord.field_get_subfield_values(author, "e")) > 0 and
        bibrecord.field_get_subfield_values(author, "e")[0] == "ed."]

    if print_links.lower() == "yes":
        editors = ['<a href="' + cfg['CFG_BASE_URL'] +
                   '/search?f=author&p=' +
                   quote(editor) +
                   '&amp;ln=' + bfo.lang +
                   '">' + editor + '</a>' for editor in editors]
