...
class AccROLE(db.Model):
    ...
    def firerole_def_ser(self, value):
        """Set firerole_def_ser."""
        # serialize(compiled rule)
        self._firerole_def_ser = bytearray(serialize(value)) \
            if value not in [CFG_ACC_EMPTY_ROLE_DEFINITION_SER, None] \
    ...
    def firerole_def_src(self, value):
        ...
        self.firerole_def_ser = compile_role_definition(value) \
            if value not in [CFG_ACC_EMPTY_ROLE_DEFINITION_SRC, None] \
            else CFG_ACC_EMPTY_ROLE_DEFINITION_SER
        # set new value
    ...
    def delete(cls, *criteria, **filters):
        ...
        objs = cls.query.filter(*criteria).filter_by(**filters).all()
        for obj in objs:
            if SUPERADMINROLE != obj.name or force:
                db.session.delete(obj)
...
class AccAuthorization(db.Model):
    ...
    @classmethod
    def factory(cls, role, action=None, arguments=None):
        ...
        if not arguments or action.is_optional():
            return cls.factory_with_no_arguments(role, action)
        else:
            return cls.factory_with_arguments(role, action, arguments)

    @classmethod
    @session_manager
    def factory_with_no_arguments(cls, role, action=None):
        """Add (and insert in DB if already not exists) a new role.

        The authorization have not arguments.

        :param role: the role
        :param action: the action
        :param is_optional: if this is set to True, check that function can
            have optional arguments and add with arglistid -1 and
            id_argument -1
        :param arguments: list of arguments
        :return: list of AccAuthorization create/loaded
        """
    ...
    @classmethod
    @session_manager
    def factory_with_arguments(cls, role, action=None, arguments=None):
        """Add (and insert in DB if already not exists) a new role.

        The authorization have arguments.
        ...
        new_argumentlistid = db.session.query(
        ...
        for argument_id in argument_ids:
            auth = AccAuthorization(
                id_accROLE=role.id,
                id_accACTION=action.id,
                argumentlistid=new_argumentlistid,
                id_accARGUMENT=None
            )
            db.session.add(auth)
            auths.append(auth)
...
class UserAccROLE(db.Model):
    ...
    @classmethod
    def is_user_in_any_roles(cls, user_info, roles):
        ...
        filters = [
            ...
            UserAccROLE.id_accROLE.in_([r.id for r in roles])
        ]
