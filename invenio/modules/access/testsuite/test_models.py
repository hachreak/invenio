...
class UserAccROLETests(InvenioTestCase):
    ...
    def test_is_user_in_any_roles(self):
        """Test is_user_in_any_roles function."""
        from invenio.modules.access.models import UserAccROLE
        user_info = {'uid': self.user.id}
        assert UserAccROLE.is_user_in_any_roles(user_info, [self.iuiar_role])

        self.delete_objects([self.iuiar_useraccrole])
        assert not UserAccROLE.is_user_in_any_roles(user_info,
                                                    [self.iuiar_role])
...
class AccROLETests(InvenioTestCase):
    ...
    def test_delete_superadmin(self):
        """Test to delete superadmin."""
        from invenio.modules.access.models import AccROLE

        id = self.role_superadmin.id

        AccROLE.delete(AccROLE.id == id)
        assert AccROLE.exists(AccROLE.id == id)

        AccROLE.delete(*[AccROLE.id == id], force=True)
...
class AccAuthorizationTests(InvenioTestCase):
    ...
    def test_factory_with_no_arguments_1(self):
        ...
        from invenio.modules.access.models import AccAuthorization
        # factory_with_no_arguments() tests
        auths = AccAuthorization.factory_with_no_arguments(
            role=self.role_fwna,
    ...
    def test_factory_with_no_arguments_2(self):
        ...
        auths2 = AccAuthorization.factory_with_no_arguments(
            role=self.role_fwna,
            action=self.actions_fwna[3],
            is_optional=False
        )
    ...
    def test_factory_with_no_arguments_4(self):
        """Test factory with no arguments."""
        from invenio.modules.access.models import AccAuthorization
        # factory_with_no_arguments() tests
        auths = AccAuthorization.factory_with_no_arguments(
            role=self.role_fwna,
            action=self.actions_fwna[2],
            is_optional=True
        )
        assert len(auths) == 1
    ...
    def test_factory_with_argumnts(self):
...
class ModelDeleteTest(InvenioTestCase):
    ...
    def test_delete_objects_user(self):
        ...
        User.delete(User.id == self.user.id)
