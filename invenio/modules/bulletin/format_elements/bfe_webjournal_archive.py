...
Prints a form that redirects to specified Bulletin issue.
"""
...
def format_element(bfo, lowest_issue):
    """Print a form that redirects to specified journal issue.
    ...
    :param lowest_issue: earliest existing issue.
        If not given, find out by checking released issues
    """
        ...
        for year in range(min_year, max_year + 1):
            for number in range(1, max_issues + 1):
                ...
                issue_number = ("%0" + str(len(str(max_issues))) + "i/%i") % \
                               (number, year)
                if issue_number not in issues_by_year[year]:
                    issues_by_year[year].append(issue_number)
    ...
    archive_form = '''
    <form id="archiveselectform" class="archiveform"
        action="%(CFG_SITE_URL)s/journal/search" name="searchbyissue"
        method="get">
        <em>%(select_year_label)s </em>
        <select name="archive_year"
            onchange="document.searchbyissue.submit();">
            %(select_year_list)s
        </select>
        <br />
        <br />
    ...
    <hr />
    <form class="archiveform" action="%(CFG_SITE_URL)s/journal/search"
        name="searchbydate" method="get">
        <em>%(custom_date_label)s
            <small>(dd/mm/yyyy  -> e.g. 01/03/2006)</small>: </em>
           ...
           'select_year_list': "\n".join(
               ['<option value="%s" %s>%s</option>' %
                (year,
                 ...
                 year) for year in journal_years]),
           'select_issue_label': _("Select Issue:"),
           'select_issue_list': "\n".join(
               ['<option value="%s">%s</option>' % (issue, issue)
